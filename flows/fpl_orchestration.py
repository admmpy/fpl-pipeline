...
def fpl_weekly_orchestration(
    ...
    if predictions:
        ...
        should_run_post_dbt = dbt_result.is_success or not allow_stale_data
        if not should_run_post_dbt:
            logger.warning(
                "Skipping post-load dbt build because initial dbt failed and stale data is allowed"
            )
            pipeline_metrics["steps_completed"].append("dbt_post_load (skipped)")
        else:
            try:
